...
class PyTorchAdapter(FrameworkAdapter):
    ...
    def __init__(self, batch_size: int = 32, num_workers: int = 0,
                 ...
                 drop_last: bool = False, device: str = 'auto'):
        """
            ...
            for batch in val_data:
                ...
                if hasattr(outputs, 'dim') and outputs.dim() > 1:
                    ...
                    if targets is not None:
                        ...
                        total += targets.size(0)
                        correct += predicted.eq(targets if targets.dim() == 1 else targets_idx).sum().item()
        
        eval_metrics = {
            'val_loss': total_loss / len(val_data) if len(val_data) > 0 else 0.0
        }
...
class TorchCVRunner:
    ...
    def run(self, dataset: Any, epochs: int = 10,
            optimizer_fn: Optional[Callable] = None,
            loss_fn: Optional[Any] = None,
            scheduler_fn: Optional[Callable] = None,
            callbacks: Optional[List[Any]] = None,
            groups: Optional[np.ndarray] = None) -> CVResults:
        """

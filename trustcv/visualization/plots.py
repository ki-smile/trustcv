...
def plot_cv_splits(cv, X, y=None, groups=None, n_splits=5, figsize=(10, 6), title: Optional[str] = None):
    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        ...
        ax.scatter(range(n_samples), [i] * n_samples, 
                  c=train_mask, cmap='RdBu', vmin=0, vmax=1,
...
def plot_cv_indices(cv, X, y=None, groups=None, n_splits=5, figsize=(12, 8), title: Optional[str] = None):
    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        ...
        # Plot
        ax.bar(indices[train_mask], np.ones(train_mask.sum()), 
               color='blue', alpha=0.5, label='Train')
        ax.bar(indices[test_mask], np.ones(test_mask.sum()), 
               color='red', alpha=0.5, label='Test')
...
def plot_temporal_cv(cv, n_samples=100, n_splits=5, figsize=(12, 6), title: Optional[str] = None):
    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        ...
        ax.barh(i, len(train_idx), left=train_idx[0], 
                color='blue', alpha=0.6, height=0.6)
        # Plot test period  
        ax.barh(i, len(test_idx), left=test_idx[0],
                color='red', alpha=0.6, height=0.6)
...
def plot_grouped_cv(cv, groups, n_splits=5, figsize=(10, 6), title: Optional[str] = None):
    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        for idx in train_idx:
            group = groups[idx]
            ax.scatter(idx, i, color=group_color_map[group], 
    ...
    handles = [plt.Line2D([0], [0], marker='o', color='w', 
                         markerfacecolor=color, markersize=8, label=f'Group {g}')
...
def plot_spatial_cv(cv, coordinates, n_splits=5, figsize=(12, 8), title: Optional[str] = None):
    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        ...
        # Plot train points
        ax.scatter(coordinates[train_idx, 0], coordinates[train_idx, 1],
                  c='blue', alpha=0.5, s=20, label='Train')
        # Plot test points
        ax.scatter(coordinates[test_idx, 0], coordinates[test_idx, 1],
                  c='red', alpha=0.7, s=30, label='Test')

...
class GroupKFoldMedical(_BaseKFold):
    ...
    >>> cv = PatientGroupKFold(n_splits=5)
...
class NestedGroupedCV:
    ...
    def __init__(self, outer_cv=None, inner_cv=None):
        if outer_cv is None:
            outer_cv = PatientGroupKFold(n_splits=5)
        if inner_cv is None:
            inner_cv = PatientGroupKFold(n_splits=3)
...
class MultilevelCV:
    ...
    def split(self, X, y=None, groups=None):
        ...
        unique_groups = np.unique(level_groups)
        
        # Create k-fold split at the specified level
...
class HierarchicalGroupKFold(_BaseKFold):
    ...
    n_splits : int
        Number of folds
    hierarchy_level : str
        Level to split on ('patient', 'department', 'hospital')
    """
    
    def __init__(self, n_splits=5, hierarchy_level='patient', 
    ...
    def split(self, X, y=None, groups=None, hierarchy=None):
        ...
        y : array-like
            Labels
        groups : array-like
            Primary grouping (e.g., patient IDs)
        hierarchy : dict
        ...
        if hierarchy is None:
            # Fall back to regular grouped splitting
            gkf = GroupKFold(n_splits=self.n_splits)
            yield from gkf.split(X, y, groups)
        else:
            # Implement hierarchical splitting logic
            # This is a placeholder for more complex hierarchical logic
            raise NotImplementedError(
                "Full hierarchical splitting coming in next version"
            )

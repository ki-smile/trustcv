...
from typing import Optional, Dict, Any, Union, Tuple, List, Iterable, Callable
import warnings
...
class TrustCVValidator:
    ...
    def validate(
        ...
        *,
        ...
        for k, (tr, te) in enumerate(splitter.split(X_arr, y_arr, split_groups), 1):
            ...
            if hasattr(est, 'predict_proba'):
                try:
                    ...
                    y_score = proba[:, 1] if _np.asarray(proba).ndim == 2 and _np.asarray(proba).shape[1] > 1 else _np.asarray(proba).ravel()
                except Exception:
                    y_score = None
        ...
        need_bootstrap_rng = (self.ci_method or "bootstrap").lower() in ("bootstrap", "boot", "bstrap")
        ...
        if leakage_checker is not None:
            try:
                leak_report = leakage_checker.check(
                    X=X_arr,
                    y=y_arr,
                    groups=split_groups
                )
        ...
        scores_dict = {m: np.asarray(v, dtype=float) for m, v in per_metric_scores.items() if len(v) > 0}
    ...
    def _setup_splitter(self):
        ...
        if method_key == 'kfold':
            ...
            if namelow in (
                'stratifiedgroupkfold', 'stratified_groupkfold', 'stratified_group_kfold',
    ...
    def _check_data_leakage(
        ...
        return self._basic_integrity_checks(
            X,
            y,
            groups=patient_ids,
...
class NestedGroupedCV:
        ...
        *,
        ...
        if isinstance(scoring, str):
            if scoring == "average_precision":
                self._score_fn = lambda yt, ys: __import__('sklearn.metrics').metrics.average_precision_score(yt, ys)
            elif scoring == "roc_auc":
                self._score_fn = lambda yt, ys: __import__('sklearn.metrics').metrics.roc_auc_score(yt, ys)
            else:
                raise ValueError("scoring must be 'average_precision', 'roc_auc', or a callable(y_true, y_score)->float")
        else:
    ...
    def fit_predict(
        ...
        *,
        ...
        # Outer loop
        for k, (tr, te) in enumerate(self.outer_cv.split(X, y, **self._kw(self.outer_cv, timestamps, groups)), 1):
            X_tr, y_tr = X[tr], y[tr]
            ...
            for params in ParameterGrid(param_grid):
                scores = []
                for tr2, te2 in self.inner_cv.split(X_tr, y_tr, **self._kw(self.inner_cv, ts_tr, grp_tr)):
...
class NestedTemporalCV:
    ...
    def fit_predict(
        ...
        def _resolve_scorer(sc):
            ...
            raise ValueError("Unsupported scoring. Use 'roc_auc', 'average_precision', 'accuracy',"
        ...
        for k, (tr, te) in enumerate(self.outer_cv.split(X, y, **outer_kw), 1):
            ...
            pred = (proba >= 0.5).astype(int) if proba.ndim == 1 else (proba[:, 1] >= 0.5).astype(int)

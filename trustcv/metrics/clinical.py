...
class ClinicalMetrics:
    ...
    def format_report(self, metrics: Dict) -> str:
        ...
        report += f"Risk Stratification:    {'Yes' if sig['risk_stratification_suitable'] else 'No'}\n"
...
def calculate_nnt(sensitivity: float, specificity: float, 
                  prevalence: Optional[float] = None) -> Optional[float]:
    """
...
def calculate_clinical_significance(
    ...
    thresholds = {
        ...
        'prognostic': {
            'auc': {'excellent': 0.90, 'good': 0.80, 'acceptable': 0.70}
        }
    }

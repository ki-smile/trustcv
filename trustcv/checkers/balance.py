...
class BalanceChecker:
    ...
    def _check_group_balance(self, y: np.ndarray, 
    ...
    def check_feature_distribution(self, X: np.ndarray, 
    ...
    def check_cv_balance(self, X, y, cv_splitter, groups=None):
        ...
        if size_variation > np.mean([f['test_size'] for f in fold_stats]) * 0.1:
            warnings.append(
                "Large variation in fold sizes. "
                "This may affect performance estimates."
            )
    ...
    def generate_report(self) -> str:
        ...
        if 'class_distribution' in self.report:
            ...
            for cls, stats in self.report['class_distribution'].items():
                lines.append(f"  Class {cls}: {stats['count']} samples ({stats['percentage']:.1f}%)")

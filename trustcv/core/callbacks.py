...
class CVCallback(ABC):
    ...
    def on_epoch_end(self, epoch: int, fold_idx: int, 
                    logs: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
...
class EarlyStopping(CVCallback):
    ...
    def __init__(self, monitor: str = 'val_loss', patience: int = 5,
                 ...
                 min_delta: float = 0.0001, verbose: bool = True):
        """
    ...
    def on_epoch_end(self, epoch: int, fold_idx: int, 
        ...
        if logs is None or self.monitor not in logs:
            warnings.warn(
                f"Early stopping: monitored metric '{self.monitor}' not found in logs"
            )
            return None
    ...
    def on_epoch_end(self, epoch: int, fold_idx: int, 
        ...
        if self.save_best_only:
            if self.monitor not in logs:
                warnings.warn(
                    f"Checkpoint: monitored metric '{self.monitor}' not found"
                )
                return None
...
class RegulatoryComplianceLogger(CVCallback):
    ...
    def __init__(self, output_dir: str, study_name: str,

...
import numpy as np
import warnings
from .base import FrameworkAdapter, SklearnAdapter, CVResults
...
class UniversalCVRunner:
    ...
    def __init__(self, cv_splitter: Any, framework: str = 'auto',
    ...
    def run(self, model: Union[Any, Callable], data: Any,
        ...
        for split_indices in self.cv_splitter.split(
            ...
            if self.framework in ['pytorch', 'tensorflow', 'monai', 'jax']:
                ...
                if epochs is None:
                    epochs = 10
                    warnings.warn(
                        f"No epochs specified for {self.framework}. Using default: 10"
                    )
                ...
                for epoch in range(epochs):
                    ...
                    val_metrics = self.adapter.evaluate(
                        fold_model, val_data, loss_fn, metrics
                    )
    ...
    def run_with_hyperparameter_tuning(self, model_fn: Callable,
